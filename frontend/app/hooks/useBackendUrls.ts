...
export function useBackendUrls() {
  const { httpBase, wsBase } = useMemo(() => {
    ...
    if (typeof window === "undefined") {
      return {
        httpBase:
          process.env.NEXT_PUBLIC_BACKEND_HTTP_URL ?? `http://localhost:${defaultBackendPort}`,
        wsBase: process.env.NEXT_PUBLIC_BACKEND_WS_URL ?? `ws://localhost:${defaultBackendPort}`,
      };
    }
