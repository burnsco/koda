...
export function useRoomRealtime({
  ...
  const makePeerConnection = useCallback(
    ...
    [addVideoParticipant, closeStreamPeer, closeVideoPeer, removeStreamViewer, sendSignal],
  );
  ...
  const startBroadcast = useCallback(async (): Promise<void> => {
    ...
    if (customServer.length > 0 !== customStreamKey.length > 0) {
      onStatusNote("Enter both OBS server and stream key, or leave both blank.");
      return;
    }
    ...
    try {
      ...
      onStatusNote("Broadcast created. Start streaming from OBS with the shown credentials.");
  ...
  const stopBroadcast = useCallback(
    ...
    [clearStreamViewers, closeAllStreamPeers, fetchStreams, httpBase, onStatusNote, sessionToken],
  );
  ...
  const getRemoteVideoStream = useCallback(
    (remoteUserId: string) => videoRemoteStreamsRef.current.get(remoteUserId) ?? null,
    [],
  );
