...
describe("useRoomRealtime", () => {
  ...
  it("requires auth before starting a broadcast", () => {
    ...
    expect(onStatusNote).toHaveBeenCalledWith("Sign in before you start a stream.");
  ...
  it("requires auth before joining a video room", () => {
    ...
    expect(onStatusNote).toHaveBeenCalledWith("Sign in before joining a video room.");
    expect(onAuthRequired).toHaveBeenCalledTimes(1);
  });
});
